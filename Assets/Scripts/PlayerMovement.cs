...
public class PlayerMovement : MonoBehaviour
{
    ...
    {
        Debug.Log("Timer started/stopped.");
    }
    ...
    private void WallRunInput() //make sure to call in void Update
    {
        ...
        if (isWallForward && !grounded) StartWallrun();
        if (isWallBackward && !grounded) StartWallrun();
    }
    ...
    private void CheckForWall() //make sure to call in void Update
    {
        ...
        //leave wall run
        if (!isWallLeft && !isWallRight && !isWallForward && !isWallBackward) StopWallRun();
    }
    ...
    private void MaxWallrunTime()
    {
        if (isWallRunning)
        {
            rb.AddForce(Vector3.down * Time.deltaTime * 500 / maxWallrunTime);
        }
    }
    ...
    private void Jump()
    {
        ...
        {
            readyToJump = false;
            Debug.Log("You Wall Jumped");
            StopWallRun();

            //normal jump
            if (isWallLeft)
            {
                Debug.Log("Off left wall");
                rb.AddForce(Vector3.up * jumpForce * 0.5f);
                rb.AddForce(orientation.transform.right * jumpForce * 2f);
            }

            if (isWallRight)
            {
                Debug.Log("Off right wall");
                rb.AddForce(Vector3.up * jumpForce * 0.5f);
                rb.AddForce(-orientation.transform.right * jumpForce * 2f);
            }
            if (isWallForward)
            {
                Debug.Log("Off forward wall");
                rb.AddForce(Vector3.up * jumpForce * 0.5f);
                rb.AddForce(-orientation.transform.forward * jumpForce * 2f);
            }
            if (isWallBackward)
            {
                Debug.Log("Off backward wall");
                rb.AddForce(Vector3.up * jumpForce * 0.5f);
                rb.AddForce(orientation.transform.forward * jumpForce * 2f);
            }
